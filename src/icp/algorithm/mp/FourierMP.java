...
{	
	...
	public DecompositionCollection doMP(double[] signal, int iterations) throws IllegalArgumentException
	{
		...
		else if (Math.abs(nDouble - nLong) > ALLOWANCE)
		{
			double[] newSignal = new double[Mathematic.newMajorNumberOfPowerBase2(signal.length)];
			newSignal = Arrays.copyOf(signal, newSignal.length);
			Arrays.fill(newSignal, signal.length, newSignal.length, Mathematic.ZERO);
			signal = newSignal;
		}
